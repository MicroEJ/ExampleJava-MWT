...
public class StackContainer extends Container {
	...
	public void addChild(final Widget child) {
		stopAnimation();
		// The former visible child will be hidden, stop any animation/refresh running.
		int childrenCount = getChildrenCount();
		if (childrenCount >= 1) {
			Widget formerVisibileChild = getChild(childrenCount - 1);
			setHiddenChild(formerVisibileChild);
		}
