...
import com.microej.example.mwt.attribute.selector.AttributeValueContainSelector;

import ej.mwt.stylesheet.selector.SelectorHelper;

public class AttributeValueContainSelectorTest {
	...
	@Test
	public void test() {
		AttributeValueContainSelector selector = new AttributeValueContainSelector(ATTRIBUTE, VALUE);
		...
		// Test: the widget contains the attribute with the right value as prefix
		widget.addAttribute(ATTRIBUTE, VALUE + "xxx");
		Assert.assertTrue("Selector does not match", selector.appliesToWidget(widget));

		// Test: the widget contains the attribute with the right value as suffix
